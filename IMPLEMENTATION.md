...
| Vision/LLM extraction | **Claude (Opus/Sonnet 5) with structured output** as primary; multimodal when text OCR is insufficient | Best instruction-following for constrained JSON extraction; a single provider keeps ops simple | Local Qwen2-VL as offline fallback — *Optional*, ops cost is real |
| Model routing | Cheap model first (Sonnet-class) → escalate to Opus-class on low confidence or schema-validation failure | Cost control without accuracy loss on hard inputs | — |
